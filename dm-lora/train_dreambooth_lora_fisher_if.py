...
def compute_fisher_for_original_attn(
    ...
    for step, batch in enumerate(dataloader):
        ...
        if args.pre_compute_text_embeddings:
            encoder_hidden_states = batch["input_ids"].to(device=accelerator.device, dtype=weight_dtype)
        else:
            encoder_hidden_states = encode_prompt(
                text_encoder,
                batch["input_ids"],
...
def precompute_prompt_embeddings(args, train_dataset, tokenizer, text_encoder, device, max_length=77):
    """Encode all unique prompts with T5 once, return {prompt_str: Tensor[seq, hidden]} on CPU."""
    if args.instance_prompt and args.instance_prompt != '':
        unique_prompts = [args.instance_prompt]
    elif hasattr(train_dataset, 'prompt_list'):
        unique_prompts = list(dict.fromkeys(train_dataset.prompt_list))
    else:
        seen = []
        seen_set = set()
    ...
    if args.validation_prompt and args.validation_prompt not in unique_prompts:
        unique_prompts.append(args.validation_prompt)
    ...
    with torch.no_grad():
        for p in unique_prompts:
            ...
            emb = out[0].squeeze(0).detach().cpu().float()  # [seq, hidden]
            cache[p] = emb
    return cache
...
def main(args):
    ...
    for name, param in unet.named_parameters():
        # IF's UNet names attention params with 'attentions' (plural), SD uses 'attn1/attn2'.
        if ('attn' in name) or ('attentions' in name):
    ...
    # ---- IF: precompute T5 embeddings so we can free the 11B text encoder before Fisher ----
    prompt_embed_cache = precompute_prompt_embeddings(
        args, train_dataset, tokenizer, text_encoder, accelerator.device,
        max_length=args.tokenizer_max_length or 77,
    )
    # Free T5 memory (only UNet matters for Fisher and training)
    text_encoder = text_encoder.to('cpu')
    del text_encoder
    text_encoder = None
    gc.collect()
    torch.cuda.empty_cache()
    args.pre_compute_text_embeddings = True  # tells compute_fisher_for_original_attn to read batch["input_ids"] as embeddings
    ...
    def collate_fn(examples, with_prior_preservation=False):
        ...
        encoder_hidden_states = torch.stack([prompt_embed_cache[p] for p in prompts], dim=0)

        pixel_values = torch.stack(pixel_values)
        pixel_values = pixel_values.to(memory_format=torch.contiguous_format).float()

        batch = {
            "input_ids": encoder_hidden_states,  # pre-compute path consumes this as embeddings

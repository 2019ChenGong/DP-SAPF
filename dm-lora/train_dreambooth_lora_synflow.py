...
def compute_synflow_scores(
    ...
    """SynFlow: data-free iterative layer importance scoring via Synaptic Flow.

    Each iteration (1) makes all attention weights positive (abs), (2) feeds a
    synthetic all-ones input through the network, (3) back-propagates the sum of
    the output to get gradients, and (4) computes score = |w| * grad for each
    surviving projection key. Using positive weights and positive inputs ensures
    no sign cancellation and avoids layer collapse.  Pruning follows the same
    geometric schedule as FORCE.
    """
    unet.eval()
    ...
    if args.random_selection:
        ...
        for name, _ in unet.named_parameters():
            ...
            if key:
                fisher_keys.add(key)
        return {key: np.random.rand() for key in fisher_keys}

    # Build key → param-name mapping before accelerator wraps the model
    name_to_param = dict(unet.named_parameters())
    key_to_param_names = defaultdict(list)
    all_keys = set()
    for raw_name in name_to_param:
        clean = raw_name[7:] if raw_name.startswith('module.') else raw_name
        key = infer_fisher_key_from_param_name(clean)
        if key:
            key_to_param_names[key].append(raw_name)
            all_keys.add(key)

    # Determine synthetic input shapes from model config (truly data-free)
    bare_unet = accelerator.unwrap_model(unet)
    in_channels = bare_unet.config.in_channels
    cross_attn_dim = bare_unet.config.cross_attention_dim
    latent_h = latent_w = args.resolution // 8
    seq_len = 77  # standard CLIP token length

    device = accelerator.device
    dummy_latent = torch.ones(1, in_channels, latent_h, latent_w, device=device, dtype=weight_dtype)
    dummy_text_emb = torch.ones(1, seq_len, cross_attn_dim, device=device, dtype=weight_dtype)
    dummy_timestep = torch.full((1,), noise_scheduler.config.num_train_timesteps // 2,
                                dtype=torch.long, device=device)
    dummy_noise = torch.ones_like(dummy_latent)
    dummy_noisy = noise_scheduler.add_noise(dummy_latent, dummy_noise, dummy_timestep)

    if bare_unet.config.in_channels == dummy_noisy.shape[1] * 2:
        dummy_noisy = torch.cat([dummy_noisy, dummy_noisy], dim=1)

    class_labels = dummy_timestep if args.class_labels_conditioning == "timesteps" else None

    masks = {key: True for key in all_keys}
    current_scores = {key: 0.0 for key in all_keys}
    synflow_iterations = args.synflow_iterations

    for synflow_iter in range(synflow_iterations):
        # Temporarily make active keys' weights positive, zero out pruned keys
        saved = {}
        for raw_name, param in name_to_param.items():
            clean = raw_name[7:] if raw_name.startswith('module.') else raw_name
            key = infer_fisher_key_from_param_name(clean)
            if key is not None:
                saved[raw_name] = param.data.clone()
                if masks.get(key, False):
        ...
        unet.zero_grad()
        model_output = unet(dummy_noisy, dummy_timestep, dummy_text_emb,
                            class_labels=class_labels).sample
        if model_output.shape[1] == 6:
            model_output, _ = torch.chunk(model_output, 2, dim=1)

        # SynFlow loss: sum of all positive outputs
        loss = model_output.abs().sum()
        loss.backward()

        # Score = |w| * grad (both positive → no cancellation)
        iter_scores = {}
        for raw_name, param in name_to_param.items():
            clean = raw_name[7:] if raw_name.startswith('module.') else raw_name
            if param.grad is not None:
                key = infer_fisher_key_from_param_name(clean)
                if key is not None and masks.get(key, False):
                    iter_scores[key] = iter_scores.get(key, 0.0) + (param.data * param.grad).sum().item()

        # Restore original weights
        for raw_name, data in saved.items():
            name_to_param[raw_name].data.copy_(data)

        current_scores = iter_scores

        # Geometric pruning schedule: after all iterations keep top_k_lora fraction
        if synflow_iter < synflow_iterations - 1:
            active_keys = [k for k in all_keys if masks[k]]
            target_keep = max(1, int(len(all_keys) * args.top_k_lora ** ((synflow_iter + 1) / synflow_iterations)))
            num_to_prune = max(0, len(active_keys) - target_keep)
            if num_to_prune > 0:
                sorted_active = sorted(active_keys, key=lambda k: current_scores.get(k, 0.0))
                for key in sorted_active[:num_to_prune]:
                    masks[key] = False

        if accelerator.is_main_process:
            active = sum(1 for k in all_keys if masks[k])
            print(f"SynFlow iter {synflow_iter + 1}/{synflow_iterations}: {active} active keys")

    # --- Gather across processes and post-process ---
    keys_sorted = sorted(all_keys)
    final_scores = {}
    for k in keys_sorted:
        local_val = torch.tensor(current_scores.get(k, 0.0), device=device)
        gathered_vals = accelerator.gather(local_val)
        total_val = gathered_vals.sum().cpu().item()

        w = args.fisher_weight if keys_sorted.index(k) >= len(keys_sorted) // 2 else 1.0
        final_scores[k] = total_val * w

        if accelerator.is_main_process:
            print(f"{k}: {final_scores[k]}")
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of iterative pruning rounds for SynFlow scoring.",
    )
...
def main(args):
    ...
    print("🔍 Computing SynFlow scores for attention layers (data-free)...")

    fisher_scores = compute_synflow_scores(
        unet=unet,
        vae=vae,

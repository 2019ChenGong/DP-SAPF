...
def main(args):
    ...
    # Soft variant: instead of top-K hard 0/1 selection, save a normalized weight in
    # (eps_w, 1] for every (attention module, projection) pair. The downstream training
    # script uses this weight as a per-projection learning-rate multiplier.
    if accelerator.is_main_process:
        # Step 1: Group fine-grained scores by attention module + projection letter
        module_proj_scores = defaultdict(dict)  # {base.processor: {q: score, k: ..., v: ..., o: ...}}
        for full_key, score in fisher_scores.items():
            parts = full_key.split(".")
            if any(proj in parts[-1] for proj in ["to_q", "to_k", "to_v", "to_out"]):
                # 'to_q' -> 'q', 'to_k' -> 'k', 'to_v' -> 'v', 'to_out' -> 'o'
                proj_name = parts[-1][3:][:1]
                base_path = ".".join(parts[:-1]) + ".processor"
                module_proj_scores[base_path][proj_name] = float(score)

        # Step 2: Min-max normalize across all projections to soft weights in (eps_w, 1]
        all_scores = [s for d in module_proj_scores.values() for s in d.values()]
        smin, smax = (min(all_scores), max(all_scores)) if all_scores else (0.0, 0.0)
        eps_w = float(getattr(args, "soft_min_weight", 1e-3))
        temperature = float(getattr(args, "soft_temperature", 1.0))  # >1 sharper, <1 flatter
        ...
        def soft_w(s):
            ...
            x = (s - smin) / (smax - smin)
            x = x ** temperature
            return eps_w + (1.0 - eps_w) * x

        soft_weights = {
            base: {p: soft_w(s) for p, s in projs.items()}
            for base, projs in module_proj_scores.items()
        }

        # Step 3: Save as JSON (dict-of-dict: {module: {proj_letter: weight}})
        ...
        with open(save_path, "w") as f:
            json.dump(soft_weights, f, indent=2)

        n_mods = len(soft_weights)
        n_projs = sum(len(v) for v in soft_weights.values())
        print(f"✅ Saved soft Fisher weights for {n_mods} attention modules ({n_projs} projections) -> {save_path}")
        for base, projs in sorted(soft_weights.items()):
            print(f"  - {base}: " + ", ".join(f"{p}={w:.3f}" for p, w in sorted(projs.items())))

...
def compute_fisher_for_original_attn(
    ...
    for k in keys_sorted:
        ...
        # Score-level Gaussian noise is intentionally disabled for the Exponential
        # Mechanism variant. Privacy comes from EM sampling on top-k below; adding
        # noise to the utility here would double-count the privacy budget.
...
def main(args):
    ...
    if accelerator.is_main_process:
        # === Exponential Mechanism top-k selection ===
        # P(i) ∝ exp(score_i / (2 * sigma * sensitivity)),
        # sampled WITHOUT replacement k times. sigma = args.fisher_sigma,
        # sensitivity = args.fisher_norm (per-layer L2 clip from clip_grad_norm_).
        items = list(fisher_scores.items())
        total_num = len(items)
        ...
        denom = 2.0 * sigma * sensitivity
        if denom <= 0.0:
            # Degenerate case: fall back to deterministic top-k by raw score.
            print("⚠️  fisher_sigma * fisher_norm <= 0; falling back to deterministic top-k.")
            sorted_items = sorted(items, key=lambda x: x[1], reverse=True)
            top_k_fine_grained = set(name for name, _ in sorted_items[:top_k_count])
        else:
            names = [n for n, _ in items]
            scores = np.asarray([float(s) for _, s in items], dtype=np.float64)
            # Gumbel-trick equivalent of EM without replacement:
            # take top-k of (score_i / denom + Gumbel(0,1)).
            # This is exact: log P(i) ∝ score_i / denom.
            log_weights = scores / denom
            # Gumbel(0,1) = -log(-log(U)), U ~ Uniform(0,1)
            u = np.random.uniform(low=1e-12, high=1.0 - 1e-12, size=log_weights.shape)
            gumbel = -np.log(-np.log(u))
            perturbed = log_weights + gumbel
            if top_k_count == 0:
                top_k_fine_grained = set()
            else:
                topk_idx = np.argpartition(-perturbed, kth=top_k_count - 1)[:top_k_count]
                top_k_fine_grained = set(names[i] for i in topk_idx.tolist())
            print(f"🎲 Exponential Mechanism: sampled {len(top_k_fine_grained)} / {total_num} "
                  f"layers (sigma={sigma}, sensitivity={sensitivity}, denom={denom}).")

...
def compute_fisher_for_original_attn(
    ...
    for k in keys_sorted:
        ...
        # Score-level Gaussian noise is intentionally disabled for the PTR variant.
        # Privacy comes from noising the stability gap in the PTR test below; adding
        # score-level noise here would double-count the privacy budget.
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="PTR threshold multiplier: release top-k iff noisy gap > ptr_threshold * sigma * sensitivity.",
    )
    ...
    parser.add_argument(
        ...
        default="error",
        choices=["error", "plain"],
        help="Behavior when PTR test fails: 'error' raises RuntimeError; 'plain' falls back to non-private deterministic top-k.",
    )
...
def main(args):
    ...
    if accelerator.is_main_process:
        # === Propose-Test-Release top-k selection ===
        # 1) Propose: deterministic top-k from clean Fisher scores.
        # 2) Test:    gap = score[k-1] - score[k]; noisy_gap = gap + N(0, (sigma*sensitivity)^2)
        # 3) Release: iff noisy_gap > ptr_threshold * sigma * sensitivity.
        ...
        top_k_count = max(0, min(top_k_count, total_num))

        sigma = float(args.fisher_sigma)
        sensitivity = float(args.fisher_norm)
        noise_scale = sigma * sensitivity
        threshold = float(args.ptr_threshold) * noise_scale

        # Edge cases where the test is trivially passable / not applicable.
        if top_k_count == 0 or top_k_count == total_num:
            top_k_fine_grained = set(name for name, _ in sorted_items[:top_k_count])
            print(f"ℹ️  PTR test skipped (top_k_count={top_k_count}, total={total_num}); "
                  f"released deterministic selection.")
        else:
            score_k = float(sorted_items[top_k_count - 1][1])      # k-th largest
            score_k_plus_1 = float(sorted_items[top_k_count][1])   # (k+1)-th largest
            gap = score_k - score_k_plus_1
            if noise_scale > 0.0:
                noisy_gap = gap + float(np.random.randn()) * noise_scale
            else:
                noisy_gap = gap  # no privacy requested; trivially release.

            print(f"🧪 PTR test: clean_gap={gap:.6g}, noisy_gap={noisy_gap:.6g}, "
                  f"threshold={threshold:.6g} (sigma={sigma}, sensitivity={sensitivity}, "
                  f"ptr_threshold={args.ptr_threshold}).")

            if noisy_gap > threshold:
                top_k_fine_grained = set(name for name, _ in sorted_items[:top_k_count])
                print(f"✅ PTR passed: released top-{top_k_count} layers.")
            else:
                msg = (f"PTR failed: noisy_gap ({noisy_gap:.6g}) <= threshold "
                       f"({threshold:.6g}); the top-{top_k_count} selection is not "
                       f"certifiably stable under the chosen noise scale.")
                if args.ptr_fallback == "error":
                    raise RuntimeError(msg)
                else:
                    print(f"⚠️  {msg}\n    Falling back to deterministic top-k (NON-PRIVATE).")
                    top_k_fine_grained = set(name for name, _ in sorted_items[:top_k_count])

...
def compute_fisher_for_all_linear_layers(
    unet,
    vae,
    text_encoder,
    train_dataloader,
    ...
    # === Step 1: 收集所有 nn.Linear 层的模块路径 ===
    unwrapped_unet = accelerator.unwrap_model(unet)
    linear_module_paths = set()
    for name, module in unwrapped_unet.named_modules():
        if isinstance(module, torch.nn.Linear):
            linear_module_paths.add(name)
    
    # === Step 2: 随机选择模式处理 ===
    if args.random_selection:
        fisher = {path: np.random.rand() for path in linear_module_paths}
        return fisher

    # === Step 3: 初始化 Fisher 字典（所有线性层初始为 0）===
    fisher = {path: 0.0 for path in linear_module_paths}
    ...
    progress_bar.set_description("Computing Fisher (All Linear Layers)")

    # === Step 5: 前向 + 反向传播并累加梯度平方 ===
    for step, batch in enumerate(dataloader):
        ...
        for name, param in unet.named_parameters():
            ...
            if clean_name.endswith('.weight') or clean_name.endswith('.bias'):
                module_path = clean_name.rsplit('.', 1)[0]  # 去掉 .weight / .bias
                if module_path in linear_module_paths and param.grad is not None:
                    grad_sq = (param.grad ** 2).sum().item()
                    fisher[module_path] += grad_sq

        count += 1
        progress_bar.update(1)

    # === Step 6: 多进程 gather 与后处理 ===
    keys_sorted = sorted(linear_module_paths)  # 确保跨进程顺序一致
    final_fisher = {}
    ...
    for k in keys_sorted:
        ...
        # 权重调整（沿用原逻辑：后半部分线性层应用 fisher_weight）
...
def main(args):
    ...
    fisher_scores = compute_fisher_for_all_linear_layers(
        unet=unet,
        vae=vae,
    ...
    if accelerator.is_main_process:
        ...
        selected_attn_modules = list(set(name for name, _ in sorted_items[:top_k_count]))

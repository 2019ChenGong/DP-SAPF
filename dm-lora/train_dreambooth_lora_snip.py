...
def compute_snip_scores(
    ...
    progress_bar.set_description("Computing SNIP")
    ...
    for step, batch in enumerate(dataloader):
        ...
        # --- Accumulate SNIP sensitivity per to_q/k/v/o: |w * grad| ---
        for name, param in unet.named_parameters():
            if name.startswith('module.'):
                name = name[7:]
            if param.grad is not None:
                ...
                if fisher_key is not None:
                    # SNIP connection sensitivity: |w * grad|
                    snip_score = (param.data * param.grad).abs().sum().item()
                    if fisher_key in fisher:
                        fisher[fisher_key] += snip_score
                    else:
                        fisher[fisher_key] = snip_score
...
def main(args):
    ...
    print("🔍 Computing SNIP scores for attention layers...")

    fisher_scores = compute_snip_scores(
        unet=unet,
        vae=vae,

...
def load_attn_procs_(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], **kwargs):
    r"""
...
def load_attn_procs(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], **kwargs):
    ...
    [Parameters documentation remains unchanged...]
    """
    from diffusers.models.attention_processor import (
        AttnAddedKVProcessor,
        AttnAddedKVProcessor2_0,
    ...
    # ===== STEP 1: Infer adapter_type globally from state_dict =====
    def infer_adapter_type(state_dict):
        for key in state_dict.keys():
            if "lora" in key.lower():
                return "lora"
            elif "krona" in key.lower() or "kron" in key.lower():
                return "krona"
        # Fallback: check non-attention layers pattern
        for key in state_dict.keys():
            if "down.weight" in key and ("lora" in key.lower() or "kron" in key.lower()):
                return "lora" if "lora" in key.lower() else "krona"
        raise ValueError(
            "Could not infer adapter type from state_dict. Keys must contain 'lora' or 'krona' indicators."
        )
    ...
    model_file = None
    if not isinstance(pretrained_model_name_or_path_or_dict, dict):
        # [Model file loading logic remains unchanged...]
        if (use_safetensors and weight_name is None) or (
            weight_name is not None and weight_name.endswith(".safetensors")
        ):
            try:
    ...
    # Infer adapter type BEFORE any processing
    adapter_type = infer_adapter_type(state_dict)
    
    # ===== Rest of state_dict processing remains unchanged until LoRA branch =====
    attn_processors = {}
    non_attn_lora_layers = []
    is_lora = all(("lora" in k or "krona" in k or k.endswith(".alpha")) for k in state_dict.keys())
    is_custom_diffusion = any("custom_diffusion" in k for k in state_dict.keys())

    if is_lora:
        # [New LoRA format handling remains unchanged...]
        is_new_lora_format = all(
            key.startswith(self.unet_name) or key.startswith(self.text_encoder_name) for key in state_dict.keys()
        )
        if is_new_lora_format:
            # [Stripping unet prefix logic remains unchanged...]
            is_text_encoder_present = any(key.startswith(self.text_encoder_name) for key in state_dict.keys())
            if is_text_encoder_present:
                warn_message = "The state_dict contains LoRA params corresponding to the text encoder which are not being used here. To use both UNet and text encoder related LoRA params, use [`pipe.load_lora_weights()`](https://huggingface.co/docs/diffusers/main/en/api/loaders#diffusers.loaders.LoraLoaderMixin.load_lora_weights)."
                warnings.warn(warn_message)
        ...
        if not is_network_alphas_none and len(network_alphas) > 0:
            raise ValueError(
                f"The `network_alphas` has to be empty at this point but has the following keys \n\n {', '.join(network_alphas.keys())}"
            )
        if len(state_dict) > 0:
        ...
        # ===== STEP 2: Process each attention processor with dynamic projection detection =====
        for key, value_dict in lora_grouped_dict.items():
            attn_processor = self
            for sub_key in key.split("."):
                attn_processor = getattr(attn_processor, sub_key)

            # Handle non-attention layers (conv/linear)
            if "lora.down.weight" in value_dict or "lora_layer.down.weight" in value_dict:
                # Determine rank from down weight
                down_key = "lora.down.weight" if "lora.down.weight" in value_dict else "lora_layer.down.weight"
                rank = value_dict[down_key].shape[0]
                
                # Create appropriate LoRA layer based on module type
                if isinstance(attn_processor, LoRACompatibleConv):
                    lora = LoRAConv2dLayer(
                        in_features=attn_processor.in_channels,
                        out_features=attn_processor.out_channels,
                    ...
                    if adapter_type == "lora":
                        ...
                        raise NotImplementedError(f"Adapter type '{adapter_type}' not yet supported for non-attention layers")
                else:
                    raise ValueError(f"Module {key} is not a LoRACompatibleConv or LoRACompatibleLinear module.")

                # Clean keys and load state
                clean_dict = {k.replace("lora.", "").replace("lora_layer.", ""): v for k, v in value_dict.items()}
                lora.load_state_dict(clean_dict)
                non_attn_lora_layers.append((attn_processor, lora))
                continue  # Skip attention processor handling
            ...
            for proj_id, prefix in proj_prefixes.items():
                # Check for down/up weights using substring matching (handles variations like 'to_out.0_')
                has_down = any(prefix in k and k.endswith("down.weight") for k in value_dict.keys())
                has_up = any(prefix in k and k.endswith("up.weight") for k in value_dict.keys())
                
                if has_down and has_up:
                    projections_present.append(proj_id)
                    
                    # Extract rank from down weight
                    down_keys = [k for k in value_dict.keys() if prefix in k and k.endswith("down.weight")]
                    rank_map[proj_id] = value_dict[down_keys[0]].shape[0]
                    
                    # Extract hidden size from up weight
                    up_keys = [k for k in value_dict.keys() if prefix in k and k.endswith("up.weight")]
                    hidden_size_map[proj_id] = value_dict[up_keys[0]].shape[0]
            
            if not projections_present:
                raise ValueError(f"No valid LoRA projections found for processor {key}. Keys: {list(value_dict.keys())}")
            
            # Determine cross_attention_dim from first available projection's down weight
            cross_attention_dim = None
            for proj_id in ['k', 'v', 'q', 'o']:  # Priority order
                if proj_id in projections_present:
                    down_keys = [k for k in value_dict.keys() if proj_prefixes[proj_id] in k and k.endswith("down.weight")]
                    if down_keys:
                        _, cross_attention_dim = value_dict[down_keys[0]].size()
                        break
            if cross_attention_dim is None:
                raise ValueError(f"Could not determine cross_attention_dim for processor {key}")
            
            # Determine main hidden_size (use to_out if available, else fallback)
            hidden_size_ = hidden_size_map.get('o') or hidden_size_map.get('v') or hidden_size_map.get('q') or hidden_size_map.get('k')
            
            # Select appropriate processor class
            if is_added_kv:
                attn_processor_class = LoRAAttnAddedKVProcessor
            else:
                attn_processor_class = (
            ...
            # Create processor with DYNAMIC per-projection configuration
            processor_kwargs = {
                'hidden_size': hidden_size_,
                'cross_attention_dim': cross_attention_dim,
                'network_alpha': mapped_network_alphas.get(key),
                'adapter_type': adapter_type,
                'attn_update_unet': projections_present,  # DYNAMIC: e.g., ['k', 'v'] instead of global env var
                # Per-projection ranks (None if not present)
                'k_rank': rank_map.get('k'),
                'q_rank': rank_map.get('q'),
                'v_rank': rank_map.get('v'),
                'out_rank': rank_map.get('o'),
                # Per-projection hidden sizes
                'q_hidden_size': hidden_size_map.get('q'),
                'v_hidden_size': hidden_size_map.get('v'),
                'out_hidden_size': hidden_size_map.get('o'),
            }
            
            # Special handling for added KV processors (different param names)
            if is_added_kv:
                processor_kwargs.pop('q_hidden_size', None)
                processor_kwargs.pop('v_hidden_size', None)
                processor_kwargs.pop('out_hidden_size', None)
            
            attn_processors[key] = attn_processor_class(**processor_kwargs)
            attn_processors[key].load_state_dict(value_dict)
                    
    elif is_custom_diffusion:
        # [Custom Diffusion handling remains unchanged...]
        custom_diffusion_grouped_dict = defaultdict(dict)
        for key, value in state_dict.items():
            if len(value) == 0:
                custom_diffusion_grouped_dict[key] = {}
    ...
    # [Final device/dtype handling and layer setting remains unchanged...]
    attn_processors = {k: v.to(device=self.device, dtype=self.dtype) for k, v in attn_processors.items()}
    non_attn_lora_layers = [(t, l.to(device=self.device, dtype=self.dtype)) for t, l in non_attn_lora_layers]
    self.set_attn_processor(attn_processors)

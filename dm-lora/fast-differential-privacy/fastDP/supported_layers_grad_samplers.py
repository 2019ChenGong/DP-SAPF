...
def _clip_linear_grad(layer: nn.Linear, A: torch.Tensor, B: torch.Tensor, C) -> None:
    try:
        # print(layer.weight.grad_sample.dtype, C.dtype)
        # grad_weight = torch.einsum('b...,b->...',layer.weight.grad_sample,C.to(layer.weight.grad_sample.dtype))
        grad_weight = torch.einsum('b...,b->...',layer.weight.grad_sample.float(),C).to(layer.weight.grad_sample.dtype)
        del layer.weight.grad_sample
    except:

...
@torch.jit.script
def _linear_weight_norm_sample_sequential_ghostnorm(input, grad_output, grad_norm2, micro_batch_size: int =1, norm_weight: Tensor = None):
    """Lightweight norm computation in ghost clipping.

    Linear algebra identity trick -- Eq. 3 in the paper.
    """
    grad_weight=torch.bmm(input, input.transpose(-1, -2)) * torch.bmm(grad_output, grad_output.transpose(-1, -2))
    shape = grad_weight.shape
    new_shape: List[int] = [
        int(shape[0] // micro_batch_size),
        int(micro_batch_size)
    ]
    for i in range(1, len(shape)):
        new_shape.append(int(shape[i]))
    grad_weight = grad_weight.reshape(new_shape).sum(dim=1)
    grad_norm2 = grad_norm2 + grad_weight.sum(dim=(1, 2))
    clip_factor = 1/(torch.sqrt(grad_norm2.to(input))+1e-4)
    # clip_factor = 1/(torch.sqrt(grad_norm2.to(input)).max()+1e-4) * torch.ones_like(grad_norm2)
    # print(f"clip_factor_max: {(1/clip_factor).max()} clip_factor_min: {(1/clip_factor).min()} clip_factor_std: {(1/clip_factor).std()}")
    clip_factor = clip_factor.repeat_interleave(micro_batch_size, dim=0)
    if norm_weight is not None:
        clip_factor *= norm_weight
    grad_output = torch.einsum('B,B...->B...',clip_factor,grad_output)
    return torch.einsum('B...p,B...d->pd',grad_output.float(),input.float()).to(grad_output.dtype), clip_factor
...
@torch.jit.script
def _linear_weight_norm_sample_psg(input, grad_output, grad_norm2, micro_batch_size: int=1, norm_weight: Tensor = None):
    grad_weight=torch.einsum('B...d,B...p->Bpd', input.float(), grad_output.float()).to(grad_output.dtype)
    shape = grad_weight.shape
    # print(grad_weight.shape, '1', micro_batch_size)
    new_shape: List[int] = [
        int(shape[0] // micro_batch_size),
        int(micro_batch_size)
    ]
    for i in range(1, len(shape)):
        new_shape.append(int(shape[i]))
    grad_weight = grad_weight.reshape(new_shape).sum(dim=1)
    # print(grad_weight.shape, '2', torch.sum(grad_weight**2, dim=(1, 2))[:10].detach().cpu())
    grad_norm2 = grad_norm2 + torch.sum(grad_weight**2, dim=(1, 2))
    # clip_factor = torch.clamp(1/(torch.sqrt(grad_norm2.to(input))+1e-8), max=1.0)
    clip_factor = 1/(torch.sqrt(grad_norm2.to(input))+1e-4)
    if norm_weight is not None:
        clip_factor *= norm_weight
    # clip_factor = 1/(torch.sqrt(grad_norm2.to(input)).max()+1e-4) * torch.ones_like(grad_norm2)
    # print(f"clip_factor_max: {(1/clip_factor).max()} clip_factor_min: {(1/clip_factor).min()} clip_factor_std: {(1/clip_factor).std()}")
    # print(f"clip_factor_max: {clip_factor.max()} clip_factor_min: {clip_factor.min()}")
    # print(f"ori_grad_max: {grad_weight.max()} ori_grad_min: {grad_weight.min()}")
    return torch.einsum('B,B...->...',clip_factor.float(),grad_weight.float()).to(grad_output.dtype), clip_factor
...
def _layer_norm_grad_sample_same_dim(input, grad_norm2, grad_weight):
    ...
    clip_factor = 1/(torch.sqrt(grad_norm2.to(input))+1e-4)
    return torch.einsum('B,B...->...',clip_factor.float(),grad_weight.float()).to(grad_weight.dtype), clip_factor
...
class LinearFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            grad_input = torch.einsum('B...p,pd->B...d',grad_output.float(),weight.float()).to(grad_output.dtype)
        ...
        if bias is not None and ctx.needs_input_grad[2]:
            if bias.per_sample_clip==0:
                ...
                if hasattr(weight, 'micro_batch_size'):
                    grad_bias = grad_bias.view(grad_bias.shape[0]//weight.micro_batch_size, weight.micro_batch_size, *grad_bias.shape[1:]).sum(dim=1)
                grad_norm2=grad_bias.norm(2, dim=1)**2
        else:
            grad_norm2=torch.zeros(grad_output.shape[0] // weight.micro_batch_size if hasattr(weight, 'micro_batch_size') else grad_output.shape[0],device=grad_output.device)
        ...
        if ctx.needs_input_grad[1]:
            if weight.per_sample_clip==0:
                ...
                if 2*(T**2) > weight.numels:
                    grad_weight, clip_factor=_linear_weight_norm_sample_psg(input,grad_output,grad_norm2,weight.micro_batch_size if hasattr(weight, 'micro_batch_size') else 1, weight.grad_reweigh if hasattr(weight, 'grad_reweigh') else torch.ones(grad_output.shape[0], device=weight.device))
                elif input.dim()==3:
                    grad_weight, clip_factor=_linear_weight_norm_sample_sequential_ghostnorm(input,grad_output,grad_norm2,weight.micro_batch_size if hasattr(weight, 'micro_batch_size') else 1, weight.grad_reweigh if hasattr(weight, 'grad_reweigh') else torch.ones(grad_output.shape[0], device=weight.device))
                else:
                    grad_weight, clip_factor=_linear_weight_norm_sample_non_sequential_ghostnorm(input,grad_output,grad_norm2)
                # print(grad_weight.shape, torch.sum(grad_weight**2).sqrt(), weight.micro_batch_size)
        ...
        return grad_input, grad_weight * weight.clip_norm, grad_bias * weight.clip_norm if grad_bias is not None else grad_bias
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[1]:
            ...
            if weight.per_sample_clip==0:
                ...
                if 2*(T**2) > weight.numels:
                    grad_weight, clip_factor=_linear_weight_norm_sample_psg(input_unfolded,grad_output_flattened,grad_norm2, weight.micro_batch_size if hasattr(weight, 'micro_batch_size') else 1, weight.grad_reweigh if hasattr(weight, 'grad_reweigh') else torch.ones(grad_output.shape[0], device=weight.device))
                else:
                ...
                if bias is not None and ctx.needs_input_grad[2]:
                    grad_bias = torch.einsum('B,B...->...',clip_factor.float(),grad_bias.float()) / math.sqrt(bias.n_layers)+ torch.randn_like(bias, device= bias.device, dtype= bias.dtype)* bias.noise
                    #grad_bias.nan_to_num_()
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[2]:
            ...
            if weight.per_sample_clip==0:
                ...
                if bias is not None and ctx.needs_input_grad[3]:
                    grad_bias = torch.einsum('B,B...->...',clip_factor.float(),grad_bias.float()).to(grad_bias.dtype) / math.sqrt(bias.n_layers) + torch.randn_like(bias, device= bias.device, dtype= bias.dtype)* bias.noise
                    #grad_bias.nan_to_num_()
...
def replace_lora_Linear(module):
    ...
    for layer_str in dir(module):
        layer = getattr(module, layer_str)
        if type(layer) == LoRACompatibleLinear and requires_grad(layer):
            new_layer = DPLinear(in_features=layer.in_features, out_features=layer.out_features, 
                               bias=(layer.bias!=None), device=layer.weight.device, dtype=layer.weight.dtype)
            new_layer.weight = layer.weight
            new_layer.bias = layer.bias

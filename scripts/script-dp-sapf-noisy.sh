...
OUTPUT_DIR="../exp/noisy_${subjects}_4096bs_1ksteps_eps${eps}" # Where to save the model
...
accelerate launch train_dreambooth_lora_noisy.py \
    --pretrained_model_name_or_path=$MODEL_NAME \
    --instance_data_dir=$subjects \
    --bench_path=$data_path \
    --output_dir=$OUTPUT_DIR \
...
python generate_sd_bench.py --batch_size 30 --data_name $subjects --output_dir $OUTPUT_DIR"/"$lower_name --num 60000 --target_size $sensitive_resolution --model_id $MODEL_NAME --gen_size $model_resolution
